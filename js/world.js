...
const SKY_ISLAND = { x: -350, z: 480, y: 104, r: 48 };
...
class World {
  ...
  stampSkyIsland(chunk) {
    ...
    // 島のかたまりを 1 つ刻印する (上面はゆるいドーム, 下面はすぼまる岩盤)
    const stampBlob = (cxw, czw, rr, baseY, domeH, arenaR) => {
      for (let lz = 0; lz < CHUNK_SIZE; lz++) {
        for (let lx = 0; lx < CHUNK_SIZE; lx++) {
          ...
          const depth = Math.round((1 - d) * (rr * 0.42) * (0.65 + hash2(wx, wz, 0x55c) * 0.5));
    ...
    // --- 本島 (半径 sr) と衛星の小島 ---
    stampBlob(sx, sz, sr, sy, 7, 0.35);
    stampBlob(ix, iz, ir, sy + 2, 3, 0);

    const floorY = sy + 7; // アリーナの床の高さ

    // --- 帰りのスカイポータル (アリーナ西側) ---
    const pfx = sx - 14;   // ポータル面の x 中心 (2セル: pfx, pfx+1)
    for (let x = pfx - 1; x <= pfx + 2; x++) {
      set(x, floorY, sz, B.MOSSY_STONE_BRICK); // 土台 (床と面一)
    ...
    // --- 照明の柱 (アリーナの四隅と縁) ---
    for (const [dx, dz] of [[-10, -10], [10, -10], [-10, 10], [10, 10], [0, -14], [0, 14]]) {
