...
function buildTextureAtlas(seed) {
  ...
  // クリーパーのスキンと顔は, デザイン画から起こした 16x16 のドット絵で描く
  // (他のテクスチャと同様, 実行時にコードから生成する)。
  ...
  const creeperPx = (ch) => {
    ...
    const c = CREEPER_PAL[+ch];
    return px(c[0], c[1], c[2], jitter(1, 0.05));
  };
  // 胴体・脚・頭の側面に貼る迷彩模様
  const CREEPER_SKIN_ART = [
    "1014400044111111",
    "2100000044221110",
    "220##00044220100",
    "000##21000##1012",
  ...
  // 四角い両目 + T 字の口と牙 (デザイン画どおりのドット配置)
  const CREEPER_FACE_ART = [
    "2240122443311123",
    "0111122113311211",
    "0012222113344311",
    "0111112221023023",
  ...
  paintTile(TILE.MOB_CREEPER_FACE, (x, y) => creeperPx(CREEPER_FACE_ART[y][x]));
