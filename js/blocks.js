...
// 木の種類: 白樺 (明るい黄緑) / ダークオーク (暗い緑) の葉。
// ワールド生成と苗木の成長で樹種ごとに使い分ける
defBlock(B.BIRCH_LEAVES, "birch_leaves", "白樺の葉",
  [TILE.BIRCH_LEAVES, TILE.BIRCH_LEAVES, TILE.BIRCH_LEAVES],
  { opaque: false, hardness: 0.25, drops: null });
...
// 壁掛け松明 (本家同様に側面へ設置できる)。torchDir は「支えの壁からセルへ
// 向かう方向」(= 松明が傾いて突き出す方向)。壊すと通常の松明をドロップ
const TORCH_WALL_DIRS = [
  [B.TORCH_WALL_N, 0, -1, "north"],   // 壁は z+1 側, 松明は -z へ傾く
  [B.TORCH_WALL_E, 1, 0, "east"],     // 壁は x-1 側, 松明は +x へ傾く
  [B.TORCH_WALL_S, 0, 1, "south"],
  [B.TORCH_WALL_W, -1, 0, "west"],
];
for (const [twId, tdx, tdz, dirName] of TORCH_WALL_DIRS) {
  defBlock(twId, "torch_wall_" + dirName, "松明 (壁掛け)",
    [TILE.TORCH, TILE.TORCH, TILE.TORCH],
    { opaque: false, solid: false, emissive: true, hardness: 0.05, drops: B.TORCH });
  BLOCKS[twId].torch = true;
  BLOCKS[twId].torchDir = [tdx, tdz];
}
...
for (const b of BLOCKS) {
  ...
  if (b.emissive) LIGHT_LUT[b.id] = b.torch ? 14 : 15;   // 松明 (床置き/壁掛け) は光量14
